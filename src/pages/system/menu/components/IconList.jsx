...
const IconList = (props) => {
  ...
  const [iconStr, setIconStr] = useState(iconStr1);
  ...
  const renderFooter = () => {
    return (
      <>
        ...
        <Button type="primary" onClick={() => handleIconVisible(false)}>
          完成
        </Button>
      </>
    );
  ...
  return (
    ...
    >
      ...
      >
        {iconStr.split(',').map((item) => (
          <div key={item}>{iconEnum[item]}</div>
        ))}
      </Card>
    </Modal>
  );
